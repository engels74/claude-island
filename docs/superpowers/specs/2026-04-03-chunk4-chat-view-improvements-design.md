...
- `Settings.swift` -- Add `chatPanelWidthFraction` key (Double, default 0.5) to private `Keys` enum + static computed property using `defaults.double(forKey:)` getter / `defaults.set(newValue, forKey:)` setter
